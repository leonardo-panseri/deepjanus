...
class ParallelEvaluator(Evaluator):
    ...
    def __init__(self, num_workers: int):
        ...
        self.stop_workers_event: SyncEvent = self.manager.Event()
        ...
        self.process_pool: ProcessPool = Pool(num_workers, self.init_worker, (self.init_args_queue,))
    ...
    @staticmethod
    def evaluate_member_parallel(member: 'Member', stop_workers_event: SyncEvent) -> 'Member':
        raise NotImplementedError()
    ...
    def _on_eval_complete(self, member: 'Member'):
        ...
        with self.done_condition:
            ...
            self.current_evals.pop(member.name)
            log.debug(f'Other members being evaluated: {list(self.current_evals.keys())}')
            ...
            if self.stop_workers_event.is_set():
                log.debug(f'Ignoring results for {member} because stop condition has been reached')
                if not self.current_evals:
                    log.debug(f'{member.name} is the last being evaluated, waking up main thread')
                    self.done_condition.notify()
                return
            ...
            # Else stop the evaluation
                ...
                log.debug(f'Stopping condition reached with {member.name}, waiting for all workers to stop')
                self._stop_workers()
    ...
    def _evaluate_member(self, member: 'Member'):
        ...
        log.info(f'{member} evaluation start')
        self.current_evals[member.name] = True
